    ...
    def image_callback(self, image_raw):
        ...
        for i, det in enumerate(pred):
            ...
            im0 = annotator.result()
                ...
                #im1 = cv2.resize(im0, dsize=(448,256), interpolation=cv2.INTER_AREA)
                #gst_out = "appsrc ! videoconvert ! nvvidconv ! video/x-raw(memory:NVMM), format=(string)I420 ! nvv4l2h264enc profile=2 preset-level=1 control-rate=1 bitrate=10000000 ! video/x-h264, stream-format=(string)byte-stream ! h264parse ! flvmux ! rtmpsink location=rtmp://192.168.0.19/live/test"
                #out = cv2.VideoWriter(gst_out, cv2.CAP_GSTREAMER, 0, float(5.0), (int(448),int(256)))
                #if not out.isOpened():
...
class yolov5_ros(Node):
    def __init__(self):
        ...
        self.sub_image = self.create_subscription(Image, 'zed2i/zed_node/rgb/image_rect_color', self.image_callback,10)
        ...
        self.declare_parameter('weights', str(ROOT) + '/config/best_cross_tiny.pt') #!
        self.declare_parameter('data', str(ROOT) + '/data/cross.yaml') #!
